-- Fix: "column reference billing_source is ambiguous".
--
-- reserve_griffineye_usage declares an OUT column named billing_source via
-- RETURNS TABLE, which shadows ai_usage_log.billing_source inside the body. The
-- unqualified reference in the monthly-usage count made every reservation fail.
-- Qualifying the column with a table alias resolves it.
--
-- reserve_griffin_vision_usage delegates here, so this fixes photo extraction too.
...
begin
  ...
  -- Aliased so billing_source resolves to the table column, not the OUT param.
...
-- release_griffin_vision_usage has the same shadowing hazard on billing_source,
-- though it returns void; alias its reads for consistency and safety.
...
declare
  ...
  v_billing_source text;
begin
  select l.organization_id, l.billing_source
