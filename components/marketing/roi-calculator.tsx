  ...
  return (
    <div className="grid gap-8 md:grid-cols-2">
      <div className="rounded-2xl border border-border bg-card p-6">
        <h2 className="text-base text-foreground">Tell us about your operation</h2>
        <div className="mt-5 flex flex-col gap-4">
          <label className="flex flex-col gap-1.5">
            <span className="font-mono text-xs uppercase tracking-[0.08em] text-muted-foreground">Number of assets tracked</span>
            <input
              type="number"
              value={assetCount}
          ...
          <label className="flex flex-col gap-1.5">
            <span className="font-mono text-xs uppercase tracking-[0.08em] text-muted-foreground">Average fully-loaded hourly rate ($)</span>
            <input
              type="number"
              value={hourlyRate}
          ...
          <label className="flex flex-col gap-1.5">
            <span className="font-mono text-xs uppercase tracking-[0.08em] text-muted-foreground">Average asset value ($)</span>
            <input
              type="number"
              value={assetValue}
      ...
      <div className="rounded-2xl border border-border bg-card p-6">
        <h2 className="text-base text-foreground">Estimated annual savings</h2>
        {result ? (
          <>
            ...
            <div className="mt-6 flex flex-col gap-4">
              <div className="rounded-lg border border-border p-4">
                <p className="font-mono text-xs uppercase tracking-[0.08em] text-muted-foreground">Time saved</p>
                <p className="mt-1.5 text-sm text-foreground">
                  ~{fmt(result.hoursSaved)} hours/year, worth{' '}
                  <span className="font-mono text-foreground">{fmtMoney(result.timeSavings)}</span>
                </p>
              ...
              <div className="rounded-lg border border-border p-4">
                <p className="font-mono text-xs uppercase tracking-[0.08em] text-muted-foreground">Loss avoided</p>
                <p className="mt-1.5 text-sm text-foreground">
                  Estimated exposure of <span className="font-mono text-foreground">{fmtMoney(result.annualLossExposure)}</span>/year,
                  with <span className="font-mono text-foreground">{fmtMoney(result.lossSavings)}</span> avoidable
                </p>
                <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
