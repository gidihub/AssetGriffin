      ...
      <table className="w-full min-w-[560px] border-collapse text-left">
        <thead>
          <tr className="bg-secondary">
            <th className="p-4 text-xs font-bold uppercase tracking-wide text-muted-foreground">Category</th>
            <th className="p-4 text-xs font-bold uppercase tracking-wide text-primary">AssetGriffin</th>
            <th className="p-4 text-xs font-bold uppercase tracking-wide text-muted-foreground">{competitorName}</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row, i) => (
            <tr key={row.label} className={i % 2 === 1 ? 'bg-secondary/50' : undefined}>
              <td className="border-t border-border p-4 text-sm font-semibold text-foreground">{row.label}</td>
