    ...
    <main className={`app-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
      <aside className="sidebar">
        ...
        <div className="sidebar-bottom">
          ...
          <div className="upgrade-card"><div className="upgrade-icon"><Compass size={16} /></div><strong>Make your next move</strong><p>Compass is ready to process your first 100 assets.</p><button onClick={() => setShowIntake(true)}>Ask Compass <ArrowUpRight size={14} /></button></div>
      ...
      <section className="content-area">
        ...
        <div className="page-content">
          ...
          <div className="bottom-grid"><section className="panel audit-card"><div className="panel-header"><div><h2>Q3 physical audit</h2><p>New York HQ · Due September 30</p></div><span className="audit-percent">78%</span></div><div className="audit-progress"><i style={{ width: '78%' }} /></div><div className="audit-meta"><span><Check size={14} /> 312 verified</span><span>89 remaining</span></div><button className="full-button light" onClick={() => setActiveNav('Audits')}>Continue audit <ArrowUpRight size={14} /></button></section><section className="panel ai-card"><div className="ai-glow"><Compass size={19} /></div><div><span className="eyebrow coral-eyebrow">COMPASS AI</span><h2>Turn a photo into an asset.</h2><p>Compass reads your spreadsheet and maps it to the right fields automatically — or snap a label and let it extract the details for your review.</p><button className="button dark small" onClick={() => setShowIntake(true)}>Ask Compass <ArrowUpRight size={14} /></button></div><div className="ai-scan"><QrCode size={50} strokeWidth={1.2} /><span>Scan anything</span></div></section></div>
          </>}
        </div>
      </section>

      {showIntake && <Modal title="Compass asset intake" onClose={() => setShowIntake(false)}><IntakeModal onComplete={() => { setShowIntake(false); announce('Asset draft created for review.') }} /></Modal>}
...
function IntakeModal({ onComplete }: { onComplete: () => void }) { const [stage, setStage] = useState<'upload' | 'review'>('upload'); return stage === 'upload' ? <div className="modal-body"><div className="dropzone" onClick={() => setStage('review')}><div className="drop-icon"><ImagePlus size={24} /></div><strong>Drop a photo or scan here</strong><span>Upload a device label, barcode, or serial number photo.</span><button className="button primary small"><CloudUpload size={15} /> Choose photo</button><small>JPG, PNG up to 10 MB</small></div><div className="workflow-note"><Compass size={16} /><span><strong>Compass extracts, you approve.</strong> We’ll never add an unverified asset to your database.</span></div></div> : <div className="modal-body"><div className="review-preview"><div className="mock-device"><LaptopGlyph /></div><div><span className="eyebrow">EXTRACTED FROM PHOTO</span><h3>Looks like a Dell Latitude 5440</h3><p>We found 4 fields with high confidence.</p></div><span className="confidence">98% confident</span></div><div className="extracted-fields"><label>Manufacturer<input defaultValue="Dell" /></label><label>Model<input defaultValue="Latitude 5440" /></label><label>Serial number<input defaultValue="7XK91P2" /></label><label>Asset category<select defaultValue="Laptop"><option>Laptop</option><option>Monitor</option><option>Equipment</option></select></label></div><div className="workflow-note"><ShieldCheck size={16} /><span>Duplicate check passed. No matching serial number found in Summit Tech.</span></div><button className="button primary full-width" onClick={onComplete}><Check size={16} /> Create asset draft</button></div> }
function MigrationModal({ onComplete }: { onComplete: () => void }) { return <div className="modal-body"><div className="migration-callout"><FileSpreadsheet size={22} /><div><strong>Bring your existing inventory.</strong><span>Upload a CSV or Excel file and AssetGriffin will map the fields for you.</span></div></div><div className="migration-steps"><div className="migration-step complete"><span>1</span><div><strong>Upload your export</strong><small>AssetTiger, Excel, or CSV accepted</small></div><Check size={17} /></div><div className="migration-step active-step"><span>2</span><div><strong>Review with Compass</strong><small>Compass found 18 matching columns</small></div><ChevronDown size={17} /></div><div className="migration-step"><span>3</span><div><strong>Validate and import</strong><small>Preview before anything is saved</small></div></div></div><div className="mapping-preview"><div><span>YOUR COLUMN</span><span>ASSETGRIFFIN FIELD</span></div><p><strong>Asset Tag</strong><ArrowUpRight size={13} /><b>Asset number</b></p><p><strong>Checked Out To</strong><ArrowUpRight size={13} /><b>Assigned person</b></p><p><strong>Site</strong><ArrowUpRight size={13} /><b>Location</b></p></div><button className="button primary full-width" onClick={onComplete}><Check size={16} /> Preview 1,284 records</button></div> }
